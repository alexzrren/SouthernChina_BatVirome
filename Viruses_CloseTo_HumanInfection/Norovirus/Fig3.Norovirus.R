...
tr <- read.iqtree('/Users/zirui/BGI_Projects/GIZ_SouthernChinaBats/CloseToHuman/Norovirus.treefile') 
...
pubseq_info <- read_excel('/Users/zirui/BGI_Projects/GIZ_SouthernChinaBats/CloseToHuman/华南进化树宿主信息.xlsx', sheet = 3) |>
...
p2 <- ggtree(tr_midpointed) +
  xlim(0,5)

p2 <- p2 %<+% allseqinfo +
  geom_tiplab(mapping = aes(x=x+0.05,label=labelanno, color=source)) +
  geom_tippoint(mapping = aes(color=source)) +
  scale_color_manual(values=c('#15CD0C','black')) +
  geom_treescale(x=0,y=13,color="black") +
  geom_nodelab(hjust=1.1 , vjust=-0.3, size=3)

ggsave('/Users/zirui/BGI_Projects/GIZ_SouthernChinaBats/CloseToHuman/Fig3.Norovirus.pdf', p2, height = 3, width = 7)
